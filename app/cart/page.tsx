...
import {
  ...
  ArrowRight, Tag, PackageX, Loader2,
} from "lucide-react";
              ...
              {cart.items.map((item) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                    ...
                    <p className="text-xs text-stone-400 dark:text-stone-500 capitalize">{item.category}</p>
            ...
            <div className="sticky top-24 space-y-4">
              ...
              ">
                ...
                <div className="space-y-2.5 pt-2 border-t border-stone-100 dark:border-stone-800">
                  <Row label="সাবটোটাল" value={`৳${cart.subtotal.toLocaleString("bn-BD")}`} />
                  {cart.discount > 0 && (
                    <Row label="ছাড়" value={`-৳${cart.discount.toLocaleString("bn-BD")}`} valueClass="text-green-600 dark:text-green-400" />
                  )}
                  <Row
                    label="ডেলিভারি চার্জ"
                    value={freeDelivery ? "বিনামূল্যে" : `৳${deliveryCharge}`}
                    valueClass={freeDelivery ? "text-green-600 dark:text-green-400 font-medium" : ""}
                  />
                  {!freeDelivery && (
                    <p className="text-xs text-amber-600 dark:text-amber-400">
                      ৳{(999 - cart.total).toLocaleString("bn-BD")} আরো কিনলে ফ্রি ডেলিভারি
                    </p>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-stone-200 dark:border-stone-700">
                  <span className="text-base font-bold text-stone-900 dark:text-white">মোট</span>
                ...
                <div className="flex items-center justify-center gap-4 pt-1">
                  {["bKash", "Nagad", "Rocket", "Cash"].map((m) => (
                    <span key={m} className="text-[10px] font-medium text-stone-400 dark:text-stone-500">{m}</span>
                  ))}
                </div>
              </div>
            </div>
